...
# --- intraday / "now" extraction (Body Compass SQL sink) -----------------------------------------
# The daily summary above reduces Garmin's per-sample arrays to aggregates. For the dashboard's
# intraday charts and the "now" snapshot we instead keep the full timestamped series. Each Garmin
# array is a list of [epoch_ms, value] pairs (HR / stress / body battery / respiration / spo2).

# (intraday-series column, payload key, array key candidates)
INTRADAY_SERIES: list[tuple[str, str, tuple[str, ...]]] = [
    ("hr_series", "heart_rates", ("heartRateValues",)),
    ("stress_series", "all_day_stress", ("stressValuesArray", "stressValues", "allDayStressValuesArray")),
    ("body_battery_series", "body_battery", ("bodyBatteryValuesArray", "bodyBatteryValues")),
    ("respiration_series", "respiration", ("respirationValuesArray", "respirationValues")),
    ("spo2_series", "spo2", ("spo2ValuesArray", "spo2Values", "spO2HourlyAverages")),
    ("steps_series", "stats", ("stepsArray", "stepValues")),
]


def sample_pairs_from_keys(value: Any, keys: tuple[str, ...]) -> list[list[float]]:
    """Extract [epoch_ms, value] pairs from a Garmin payload, preserving sample timestamps."""
    pairs: list[list[float]] = []
    for container in values_for_keys(value, keys):
        pairs.extend(extract_sample_pairs(container))
    return pairs


def extract_sample_pairs(value: Any) -> list[list[float]]:
    ...
    for item in value:
        ...
        if isinstance(item, list | tuple) and len(item) >= 2:
            timestamp = number_or_none(item[0])
            number = number_or_none(item[1])
        elif isinstance(item, dict):
            timestamp = first_number_from_keys(
                item, ("startGMT", "startTimeGMT", "timestamp", "time", "epochTimestamp", "calendarDate")
            )
            number = first_number_from_keys(item, ("value", "level", "stressLevel", "bodyBattery", "heartRate"))
        if number is not None and number >= 0:
            pairs.append([timestamp, number])
    return pairs


def build_health_intraday(archive: dict[str, Any]) -> dict[str, Any]:
    """Full timestamped time-series for one day, for the dashboard's intraday charts."""
    payloads = archive.get("payloads") if isinstance(archive.get("payloads"), dict) else {}
    series: dict[str, list[list[float]]] = {}
    for column, payload_key, array_keys in INTRADAY_SERIES:
        series[column] = sample_pairs_from_keys(payloads.get(payload_key), array_keys)
    sample_counts = {column.removesuffix("_series"): len(pairs) for column, pairs in series.items()}
    return {"date": str(archive.get("date") or ""), "sample_counts": sample_counts, **series}


def build_current_status(
    archive: dict[str, Any], *, is_partial: bool, last_synced_at: str | None = None
) -> dict[str, Any]:
    """The "now" snapshot: latest readings + the day's daily-summary scalars, one row per user."""
    summary = normalize_health_archive(archive)
    intraday = build_health_intraday(archive)
    payloads = archive.get("payloads") if isinstance(archive.get("payloads"), dict) else {}
    hr = intraday["hr_series"]
    body_battery = intraday["body_battery_series"]
    stress = intraday["stress_series"]
    last_reading_ts = max(
        (pairs[-1][0] for pairs in (hr, body_battery, stress) if pairs and pairs[-1][0] is not None),
        default=None,
    )
    return {
        "as_of_date": summary["date"],
        "latest_hr": hr[-1][1] if hr else None,
        "latest_hr_at": epoch_ms_to_iso(hr[-1][0]) if hr and hr[-1][0] is not None else None,
        "current_body_battery": body_battery[-1][1] if body_battery else None,
        "current_stress": stress[-1][1] if stress else None,
        "steps": round_or_none(
            first_number_from_keys(payloads.get("stats"), ("totalSteps", "steps", "dailyStepCount")), 0
        ),
        "resting_hr": summary["resting_hr"],
        "sleep_score": summary["sleep_score"],
        "training_readiness_score": summary["training_readiness_score"],
        "last_reading_at": epoch_ms_to_iso(last_reading_ts) if last_reading_ts is not None else None,
...
def epoch_ms_to_iso(value: Any) -> str | None:
    """Garmin sample timestamps are epoch milliseconds (some endpoints use seconds); normalize to ISO-8601 UTC."""
